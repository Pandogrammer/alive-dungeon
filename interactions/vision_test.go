...
func Vision(creature c.Creature, s g.State) interface{} {
	var distance = 2
	var position = creature.Position
	var vision []w.Position
	...
	for i := minX; i <= maxX; i++ {
		for y := -j; y < 0; y++ {
			...
			vision = append(vision, w.Position{X: i, Y: posY})
		}

		vision = append(vision, w.Position{X: i, Y: position.Y})
